 ...
 *
 * TEAM MEMBERS:
 *     Steinar EyÃ¾Ã³r Valsson, steinarv12@ru.is
 *     Ã“lafur KonrÃ¡Ã°sson, olafurk12@ru.is 
 *     Hulda LÃ¡rusdÃ³ttir, huldal12@ru.is
 * 
 * IMPORTANT: Proxy only handles GET requests.
 * It is threaded.
 */ 
...
/* Global variables */
sbuf_t sbuf;    /* Shared buffer for threads */
...
int Open_clientfd_w(char* host, int port);
...
void handle_GET(rio_t client, int serverfd, char* cgiargs, char* uri, char* filename, char* method, char* version, int port) {
    ...
    if((clientfd = Open_clientfd_w(filename, port)) < 0) {
       //Close(clientfd);
		return;
    }
...
/* A wrapper for open_clientfs_ts */
int Open_clientfd_w(char *hostname, int port) {
    int OW;
    if ((OW = open_clientfd_ts(hostname, port)) < 0) {
        if (OW == -1) {
            error("Client", "Unix error");
        }
        else {
            error("Client", "DNS error");
        }
    }
    return OW;
}
